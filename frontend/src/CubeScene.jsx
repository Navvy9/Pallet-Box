...
import './CubeScene.css';  // Import the CSS file here
...
const Cube = ({ id, width, height, length, caseName, mass, index, palletSize, onDelete, onEdit }) => {
  ...
  return (
    <group position={[x, height / 2, z]}>
      ...
      <Text position={[0, height / 2 + 0.3, 0]} fontSize={0.4} color="black">
        {caseName}
      </Text>
      <Text position={[0, -height / 2 - 0.3, 0]} fontSize={0.3} color="black">
        {mass}kg
      </Text>
      <mesh position={[0, height / 2 + 0.6, 0]} onClick={onEdit}>
      ...
      <mesh position={[0, height / 2 + 0.9, 0]} onClick={onDelete}>
        <boxGeometry args={[0.5, 0.2, 0.5]} />
